...
class _EditProfilePageState extends State<EditProfilePage> {
  ...
  TextEditingController? _yearGroup;
 TextEditingController?  _campusResidence;
  TextEditingController? favMovieController;
  TextEditingController? favfoodController;
...
Future<void> fetchuser() async {
    final response = await http.get(Uri.http
        ('127.0.0.1:5000', '/profile', {'s_id': widget.studentId}));
    if (response.statusCode == 200) {
      final json = jsonDecode(response.body);
      // print(json['studentID']);
      setState(() {
        // studentIdController.text = json['studentID'];
        studentIdController = json['studentId'];
        nameController = json['name'];
        emailController= json['email'];
        passwordController= json['password'];
        majorController= json['major'];
        dobController= json['dateOfBirth'];
        favMovieController= json['favMovie'];
        favfoodController = json['favfood'];
      });
    }
  }
Future<void> profileupdate() async {
  final url = Uri.parse('http://127.0.0.1:5000/profile?s_id=${widget.studentId}');
  final response = await http.put(
    ...
    body: jsonEncode({
      ...
      'campus_residence': _campusResidence,
